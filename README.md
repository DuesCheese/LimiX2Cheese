...
### FAQ: GUI says Excel dependency is missing but you already installed it
Usually this means the Python interpreter running the GUI is different from the one where you installed packages.
Check the GUI log line `Python interpreter: ...` (or `Python 解释器: ...`) and install into that same interpreter:
```bash
